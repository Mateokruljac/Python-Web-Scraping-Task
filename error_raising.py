...
class WebsiteNotFoundError(Exception):
    """ 
    Error occurs if START_URL does not exist and  if START_URL or END_URL does not exist in sqlite database
    Attributes:
...
class ShortestPathNotFoundError(Exception):
    ...
    def __init__(self,message  = "The Shortest path does not exists!"):
        self.message = message 
        super().__init__(message)
